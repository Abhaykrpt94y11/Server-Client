...
char filename[1000];
     int count=0;
    const char *ms="Default returning msg!Not found";
void searchfile(char *dir, int depth, char fname[])
{
    ...
    while((s_e = readdir(dp)) != NULL) 
    {
        ...
        else 
        {
            ...
            if(count==l1 && count>1)
            {
                printf("File  %s is found  in server2\n ", s_e->d_name);
                   // printf("filename is %s\n",s_e->d_name);
                    flag = 100;
                    ms ="File is found at server2";
            }
...
int main(int argc, char *argv[])
{
	...
	if(argc<2)
	{
		fprintf(stderr,"error in argument, no port provided\n");
		exit(1);
	}
    ...
    searchfile("/home/abhaykrpt/Desktop/Server2",0,buffer);
       

   /* if(flag == 50)
    {
              ms = "File is not found on server2!";
            //    break;
    }
    if(flag ==100)
    {
